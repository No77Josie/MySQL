...
SELECT UniqueCarrier,
				SUM(CASE WHEN ArrDelay<60 THEN 1 ELSE 0 END)/COUNT(*)+
                COUNT(*)/SUM(COUNT(*)) OVER() AS score
FROM ontime
...
C AS
(SELECT *,
				SUM(CASE WHEN diff<=0 THEN 1
							ELSE 0
                            END) OVER(PARTITION BY date, name) AS if_less_accurate
FROM B),
D AS(
...
FROM (
  SELECT
    A.*,
    COUNT(DISTINCT UniqueCarrier) AS carrier_count
  FROM A
  GROUP BY TailNum, DateField
) AS B
...
WITH A AS(
SELECT *,
				CASE WHEN character_id = 18 THEN 1
							ELSE 0 END AS barney_ind,
				CASE WHEN character_id = 2 THEN 1
							ELSE 0 END AS homer_ind
FROM script_lines
